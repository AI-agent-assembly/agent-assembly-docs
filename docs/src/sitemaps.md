...
| --- | --- | --- |
| [`product-promise.md`](product-promise.md) | Levels 1–3 of the promise, including the default-posture table | Closes `GAP-8` by putting a level-3 surface on the evaluator route. Opens the section until AAASM-5609 lands |
...
| Gap | Disposition |
| --- | --- |
| `GAP-1` — no surface routes all six audiences | **Closed by design.** The audience router extends `README.md`'s existing role paragraph; the six sections carry the task axis. Built by AAASM-5608 |
...
| `GAP-3` — no channel-and-platform position published | **Carried forward.** A Reference slot is defined, but the content overlaps AAASM-5609's *What Ships Today* substantially. **5609 owns it**; 5608 must not build a second matrix. Blocked on AAASM-5680 for the GHCR vocabulary |
| `GAP-4` — the evidence layer has no reader-facing surface | **Carried forward.** A Verify slot is defined; the data is AAASM-5531's and ADR 0034's T3 registry does not exist yet |
...
| `GAP-7` — SDK mounts absent from the sidebar | **Half closed, and narrower than stated.** `SUMMARY.md` cannot carry an external link at all; the recommendation routes `documentation.md` into Integrate. The checkpoint half stays L3 |
| `GAP-8` — the default-posture table is filed under *About* | **Closed.** `product-promise.md` moves to Evaluate, putting a level-3 surface on the evaluator route and one link away from the operator route |
...
their job"*. **It is not met, and no sitemap can meet it.** The `auditor`'s `AU1` — find
...
| Mobile and desktop navigation constraints are considered | [Navigation constraints](#navigation-constraints-desktop-and-mobile), with a numeric budget per surface. Two of the constraints are measured rather than assumed: part titles are not links, and `SUMMARY.md` rejects external links |
| The implementation can be divided into non-overlapping page or route tickets | [The partition](#partitioning-this-into-non-overlapping-tickets). Thirteen rows naming their own files, one recorded shared file that predates this page, and a landing order |
