#!/usr/bin/env python3
"""Enforce ADR 0033's claim vocabulary and banned-absolutes rules against Docs
Hub content. AAASM-5797 / AAASM-5599.

Adapted from agent-assembly's `scripts/check_claim_vocabulary.py` (AAASM-5679),
which implements `claim-vocabulary.md` §5.3/§5.6/§6.2/§6.3/§6.5/§6.6 in full.
Per AAASM-5795's federation-by-convention decision this is a ONE-TIME PORT,
not a shared dependency: this file and agent-assembly's now diverge
independently, each shaped for its own corpus. What is kept verbatim is kept
because the underlying problem is identical (Markdown source, the same ADR
0033 forbidden-design-7 absolutes, the same false-positive shape from quoting
a banned phrase to forbid it) -- not because of a code-sharing obligation.

What is DROPPED relative to the source, because the Docs Hub corpus does not
have it: `.jsx`/`.tsx` scanning and `design/**` (that is official-website's
design-spec surface, ADR 0025's, not this repo's), and YAML front matter
handling stays defensive-only -- mdBook has no front matter (confirmed:
`docs/scripts/check_claim_vocabulary.py`'s own repo CLAUDE.md states it, and
a leading `---` in `docs/src/**` is a thematic break, not a delimiter), so a
leading `---` here is never treated as opening one.

Run against tracked Markdown source, never build output (mdBook has no
`build/` directory shaped for this the way official-website's does; source is
authoritative and there is no MDX-style compile step to hide a phrase behind):

    python3 docs/scripts/check_claim_vocabulary.py
    python3 docs/scripts/check_claim_vocabulary.py --diff-base origin/main

Exit 0 = clean or --report-only, 1 = a blocking claim published, 2 = the
checker could not prove itself (see --selftest).

Waivers: NONE of this file's rules are waivable. `claim-vocabulary.md` §7.1
states explicitly that no waiver reaches a banned absolute (`CLAIM-ABS-*`) or
an undifferentiated verb (`CLAIM-VERB-01`) -- both are unwaivable everywhere,
not just in the repo that specification lives in. What remains waivable under
ADR 0034 Decision 10 is the D-dimension rule set, checked by a different,
not-yet-built validator this file does not implement.
"""
...
# NUL: not a clause boundary (so a masked code span cannot truncate a NEG
# window) and not whitespace (so it cannot bridge SEP across a code span).
_FILLER = "\x00"
_NEWLINE_FILLER = "."  # MUST be a clause boundary, unlike _FILLER.
...
# --------------------------------------------------------------------------
# File scope: docs/src/** plus the repo-root health files this repo actually
# carries (no local SECURITY.md -- that one is inherited from the org
# `.github` repo, per this repo's own CLAUDE.md).
# --------------------------------------------------------------------------
...
EXCLUDE_GLOBS = (
    "docs/book/**",  # mdBook build output -- gitignored, but defensive anyway
    "node_modules/**",
)
...
def _structural_ranges(text: str) -> list[tuple[int, int]]:
    ...
    severity even though it is otherwise exempt: a heading, a `title:`/
    `description:` value, or a `<meta …>` tag -- each is quoted alone
    somewhere a reader never sees the surrounding sentence's exemption label.

    mdBook has no front matter (this repo's own CLAUDE.md states it), so
    unlike the source this was ported from, a leading `---` is never treated
    as opening a front-matter block here -- only headings, title/description
    lines and <meta> tags are structural.
    """
...
# must suppress. Cases are Markdown-only (no JSX variant) -- this repo's
# corpus is pure mdBook Markdown.
# --------------------------------------------------------------------------
SELFTEST_CASES: tuple[tuple[str, str | None], ...] = (
    ...
    # mdBook has no front matter, so a leading `---` block is NOT exempt the
    # way it is in the source this was ported from -- this is the one
    # deliberate behavioural difference, asserted directly.
...
def selftest() -> int:
    ...
    if not in_scope("docs/src/adr/0034-one-product-truth-and-cross-repository-documentation-governance.md"):
        failures.append("in_scope() rejects a real docs/src/adr/ page")
    if in_scope("docs/book/index.html"):
        failures.append("in_scope() accepts mdBook build output")
